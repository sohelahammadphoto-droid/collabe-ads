  ...
  const fetchScript = async (forceMode = null) => {
    ...
    try {
      if (isAgent) {
        ...
        const result = await generateMultiAgentScript(jobPayload, openRouterKey, (msg, doneList) => {
          setAgentStatusMsg(msg);
          if (doneList) setCompletedAgents(doneList);
        });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-[#131b2e] border border-purple-500/30 rounded-2xl shadow-2xl flex flex-col max-h-[92vh] overflow-hidden">
              ...
              <h2 className="text-base font-bold text-slate-100 flex items-center gap-2">
                ...
                {aiMeta && (
                  <span className={`text-[10px] px-2.5 py-0.5 rounded-full border flex items-center gap-1 font-medium ${
                    ...
                    {aiMeta.isLive ? <CheckCircle2 className="w-3 h-3 text-emerald-400" /> : <Clock className="w-2.5 h-2.5 text-amber-400" />}
        ...
        <div className="px-6 py-2.5 bg-[#0b0f19] border-b border-slate-800 flex items-center justify-between gap-3">
          <div className="flex items-center gap-1.5 p-1 bg-slate-900 border border-slate-800 rounded-xl text-xs">
            ...
            >
              ...
              {useAgentMode && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}
            </button>
            <button
        ...
        {/* Verified Agent Team Tickmarks Banner when ready */}
        {useAgentMode && !loading && aiMeta?.isAgentMode && (
          <div className="px-6 py-2 bg-emerald-500/10 border-b border-emerald-500/30 flex items-center justify-between text-xs text-emerald-300 font-semibold">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
              <span>AI টিম সফলভাবে স্ক্রিপ্ট প্রস্তুত করেছে!</span>
            </div>
            <div className="flex items-center gap-2 text-[10px]">
              <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 rounded border border-emerald-500/40">✅ DeepSeek R1</span>
              <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 rounded border border-emerald-500/40">✅ Gemini 2.0</span>
              <span className="px-2 py-0.5 bg-emerald-500/20 text-emerald-300 rounded border border-emerald-500/40">✅ Qwen 2.5</span>
            </div>
          </div>
        )}
              ...
              <div>
                ...
                {completedAgents.length > 0 && (
                  <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
                    {completedAgents.map((ag, idx) => (
                      <span key={idx} className="px-2.5 py-1 bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 rounded-full text-xs font-semibold">
                        {ag}
                      </span>
                    ))}
                  </div>
